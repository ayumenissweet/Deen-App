...
import { adhkarData } from "../data/AdhkarData.js";
import NotifIcon from "../assets/Notif.svg";
import SettingsIcon from "../assets/Settings.svg";

const AdhkarDetail = ({ setRefresh, onOpenNotif }) => {
  const { id } = useParams();
  ...
  const handleIncrement = (itemId, maxCount) => {
    ...
    if (current < maxCount) {
      ...
      localStorage.setItem(`progress-${id}`, JSON.stringify(newCounts));
    }
  };
  ...
  // calcul progression
  const allItems = [...categoryData.quran, ...categoryData.hadith];
  ...
  const progressPercent =
    allItems.length > 0 ? (completedItems / allItems.length) * 100 : 0;
  ...
  return (
    <div className="detail-page">
      ...
      <div className="adhkar-list-grid">
        {currentList.map((item) => {
          ...
          return (
            ...
            >
              <p className="arabic-text">{item.zekr}</p>

              <div className="card-footer">
                <span className="tag-green">{item.description}</span>
              </div>
      ...
      <button className="next-button-gold" onClick={handleFinish}>
        Finish
      </button>
    </div>
  );
